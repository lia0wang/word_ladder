...
#		define INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_NO_REGISTRATION_2(TestNameClass,            \
		                                                                   ...
		                                                                   Signature,                \
		                                                                   ...)                      \
			namespace { namespace INTERNAL_CATCH_MAKE_NAMESPACE(TestName) {                           \
...
#	define INTERNAL_CATCH_TEMPLATE_PRODUCT_TEST_CASE_METHOD_2(TestNameClass,                        \
		...
		namespace { namespace INTERNAL_CATCH_MAKE_NAMESPACE(TestNameClass) {                          \
				INTERNAL_CATCH_TYPE_GEN                                                                 \
				INTERNAL_CATCH_NTTP_GEN(INTERNAL_CATCH_REMOVE_PARENS(Signature))                        \
				template<typename... Types>                                                             \
				struct TestNameClass {                                                                  \
...
#		pragma warning(disable : 4312) // Converting int to T* using reinterpret_cast (issue on x64
		                                // platform)
		...
		, m_predicate(std::forward<P>(pred)) {
			if (!m_predicate(m_generator.get())) {
				...
				if (!has_initial_value) {
					Catch::throw_exception(GeneratorException("No valid value found in filtered "
						                                       "generator"));
				}
			}
		}
		...
		, m_generator(std::move(generator)) {
			...
			if (m_chunk_size != 0) {
				...
				for (size_t i = 1; i < m_chunk_size; ++i) {
					if (!m_generator.next()) {
						Catch::throw_exception(GeneratorException("Not enough values to initialize the "
							                                       "first chunk"));
					}
					m_chunk.push_back(m_generator.get());
				}
...
namespace Catch { namespace Generators {
	...
	public:
		...
		: m_elems(first, last) {
			if (m_elems.empty()) {
				Catch::throw_exception(GeneratorException("IteratorGenerator received no valid "
					                                       "values"));
			}
		}
...
namespace Catch { namespace Benchmark {
	struct Benchmark {
		...
		void run() {
			...
			CATCH_CATCH_ALL {
				if (translateActiveException() != Detail::benchmarkErrorMsg) // benchmark errors have
...
#	endif // CLARA_CONFIG_OPTIONAL_TYPE
		...
		public:
			...
			auto set(std::string const& newName) -> ParserResult {
				...
				auto filename = (lastSlash == std::string::npos) ? newName
	...
	clara::Parser makeCommandLineParser(ConfigData& config) {
		...
		auto const setWaitForKeypress = [&](std::string const& keypress) {
			...
			if (keypressLc == "never")
				...
				return ParserResult::runtimeError("keypress argument must be one of: never, start, "
				                                  "exit or both. '"
		...
		auto cli =
		   ...
		   | Opt([&](int x) { config.abortAfter = x; }, "no. failures")["-x"]["--abortx"]("abort "
		                                                                                  "after x "
		                                                                                  "failures")
		   ...
		   | Opt(config.minDuration, "seconds")["-D"]["--min-duration"]("show test durations for "
		                                                                "tests taking at least the "
		                                                                "given number of seconds")
		   | Opt(loadTestNamesFromFile, "filename")["-f"]["--input-file"]("load test names to run "
		                                                                  "from a file")
		   ...
		   | Opt(config.listTestNamesOnly)["--list-test-names-only"]("list all/matching test cases "
		                                                             "names only")
		   ...
		   | Opt(config.libIdentify)["--libidentify"]("report name and version according to "
		                                              "libidentify standard")
		   | Opt(setWaitForKeypress, "never|start|exit|both")["--wait-for-keypress"]("waits for a "
		                                                                             "keypress "
		                                                                             "before exiting")
		   | Opt(config.benchmarkSamples, "samples")["--benchmark-samples"]("number of samples to "
		                                                                    "collect (default: 100)")
		   | Opt(config.benchmarkResamples, "resamples")["--benchmark-resamples"]("number of "
		                                                                          "resamples for the "
		                                                                          "bootstrap "
		                                                                          "(default: 100000)")
		   | Opt(config.benchmarkConfidenceInterval,
		         "confidence interval")["--benchmark-confidence-interval"]("confidence interval for "
		                                                                   "the bootstrap (between 0 "
		                                                                   "and 1, default: 0.95)")
		   | Opt(config.benchmarkNoAnalysis)["--benchmark-no-analysis"]("perform only measurements; "
		...
		void enforceNotReservedTag(std::string const& tag, SourceLineInfo const& _lineInfo) {
			CATCH_ENFORCE(!isReservedTag(tag),
			              "Tag name: [" << tag << "] is not allowed.\n"
			                            << "Tag names starting with non alphanumeric characters are "
			                               "reserved\n"
			                            << _lineInfo);
		}
	} // namespace
